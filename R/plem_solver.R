#' Solving a PLEM model using Rglpk
#'
#' @description A wrapper to load a PLEM model and solve it using Rglpk.
#'
...
#'
plem_solver <- function(variables = list(), verbose = T, want_0 = F, math_txt = F){
  ...
  lp_mp <- function(){
    ...
    sol <- Rglpk_solve_LP(obj=model$objective, mat=model$constraints[[1]],
                          ...
                          verbose = verbose, tm_limit = 10000) #crew peta igualment, mira el seu arxiu
    sol <- sol[1:3]
